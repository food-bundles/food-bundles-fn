...
export function EditOrderModal({ open, onClose, order, onSaved }: EditOrderModalProps) {
  ...
  useEffect(() => {
    if (order && open) {
      ...
      const editableItems: EditableItem[] = orderItems.map((item: any) => ({
        tempId: item.id,
        orderItemId: item.id,
  ...
  const orderNumber = data.orderNumber || order.orderId;
      ...
      <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        {!canEdit && (
          <div className="bg-orange-50 border border-orange-200 rounded-md p-3 text-sm text-orange-800">
            This order cannot be edited because its status is <strong>{orderStatus}</strong>.
          ...
          <div className="bg-gray-50 p-4 rounded-lg border space-y-2">
            ...
            {discount > 0 && (
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium text-red-600">-{discount.toLocaleString()} RWF</span>
              </div>
            )}
